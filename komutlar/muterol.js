...
if(args[0] == "bilgi") {
    ...
    .setTitle('Mute rol nedir?')
    .setURL('https://discord.gg/W2ZjTfQBBG')
    .setDescription(`Mute rol sistemlerimizde ayarladığınız şeylerde tehlikeli bir durum algılanırsa o kişiye verilecek roldür.`)
return message.channel.send(engin)
}
if(args[0] == "ayarla") {
let kontrol = db.fetch(`muterol_${message.guild.id}`)
...
if(!kontrol) {
    ...
    if(!engin) return message.channel.send('Lütfen mute rolünü etiketleyin')
db.set(`muterol_${message.guild.id}`, engin.id)
const embeddd = new discord.MessageEmbed()
.setTitle('Başarılı işlem')
.setURL('https://discord.gg/W2ZjTfQBBG')
.setDescription('Mute rol ayarlandı')
...
if(args[0] == "sıfırla") {
    ...
    if(!kontrol) return message.channel.send(embedd)
    if(kontrol) {
    db.remove(`muterol_${message.guild.id}`)
    const embeddd = new discord.MessageEmbed()
    .setTitle('Başarılı işlem')
    .setURL('https://discord.gg/W2ZjTfQBBG')
    .setDescription('Mute rol sıfırlandı')
    return message.channel.send(embeddd)
    }
}
}
